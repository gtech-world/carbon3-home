...
export function MobleInventoryLevel1Item(p: { data: any }) {
  const { data } = p;
  ...
  return (
    ...
    >
      ...
      <div className="w-full whitespace-nowrap text-sm mt-[.625rem]">{`${data.carbon_emission}kg / ${data.quality}%`}</div>
...
export function MobileInventoryBreakdown(p: { data: any[] }) {
  const { data } = p;
  return (
