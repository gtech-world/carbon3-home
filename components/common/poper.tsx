...
export function PoperMenu(p: HTMLAttributes<HTMLDivElement> & PoperMenuProps) {
  ...
  return (
    <div {...other} style={{ position: isMobile ? "initial" : "relative" }} className={classNames(className)} ref={ref}>
      <div className="flex" onClick={() => toggleShow()}>{children}</div>
      {show && menus.length > 0 && (
        <div
        ...
        >
          ...
          <div className="py-[.375rem] w-full bg-white mt-[.625rem] mo:mt-[.625rem] rounded-lg z-10 relative">
            {menus.map((item, i) => (
              <div
                key={`poper_menu_item${i}`}
                className="flex items-center py-3 px-4 text-black hover:text-green-2 cursor-pointer"
                onClick={() => onClickItem(item)}
              >
                {!!item.icon && <div className="text-2xl">{item.icon}</div>}
                <div className="text-base font-medium ml-3">{item.text}</div>
              </div>
            ))}
          </div>
        </div>
